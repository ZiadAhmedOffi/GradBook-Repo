...
Figure~\ref{fig:lung_cancer_symptoms} summarizes several common clinical manifestations associated with lung cancer.
...
\begin{figure}[H]
	...
	\fbox{
		\parbox{0.9\textwidth}{
			\centering
			\vspace{1.5cm}
			\textbf{Placeholder for Figure: Common Symptoms and Signs of Lung Cancer}\\[0.5cm]
			Suggested illustration showing a human body diagram highlighting:
			persistent cough, hemoptysis, chest pain, dyspnea, fatigue,
			unexplained weight loss, hoarseness, and metastatic symptoms.
			\vspace{1.5cm}
		}
	}
	\caption{Illustration of common symptoms and clinical signs associated with lung cancer.}
	\label{fig:lung_cancer_symptoms}
\end{figure}
...
\begin{figure}[H]
	...
	\fbox{
		\parbox{0.9\textwidth}{
			\centering
			\vspace{1.5cm}
			\textbf{Placeholder for Figure: CT Imaging Examples of Lung Cancer}\\[0.5cm]
			Suggested image panel containing:
			(a) Normal chest CT scan,
			(b) Benign pulmonary nodule,
			(c) Malignant pulmonary nodule,
			(d) Advanced lung tumor with surrounding tissue involvement.
			Lesions should be annotated with arrows or bounding markers.
			\vspace{1.5cm}
		}
	}
