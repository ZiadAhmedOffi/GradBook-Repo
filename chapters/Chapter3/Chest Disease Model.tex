...
\begin{table}[htbp]
	...
	\begin{tabular}{|l|l|}
		...
		\hline
		Early stopping monitor & val\_dice\_coef (patience = 5, mode = max) \\
		\hline
		LR reduction & ReduceLROnPlateau (factor = 0.1, patience = 5, min LR = $1 \times 10^{-7}$) \\
		\hline
		Precision policy & Mixed float16 (TensorFlow AMP) \\
		\hline
		Training / validation split & 85\% / 15\% (6,106 Darwin images) \\
		\hline
		Best model checkpoint & lung\_segmentation\_darwin\_montgomery\_shenzhen.h5 \\
		\hline
	\end{tabular}
\end{table}
